...
app.post("/api/customer/items/:itemId/purchases", function(req, res) {
  ...
  var money = 35;
  if(newInventory === 0){
    vendingData.status = "fail";
    res.json({vendingData});
  }
  
  if (money >= itemInformation.cost) {
    let change = money - itemInformation.cost;
    itemInformation.quantity = itemInformation.quantity - 1;
    itemInformation.change = change;
    res.json({itemInformation});

  } 
   
    vendingData.status = "fail";
    res.json({itemInfomation});
